...
//
//
// This file implements the interfaces that rvex uses to lower LLVM code into
// a selection DAG.
//
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
// Implementation of rvex CallingConv custom function.
//===----------------------------------------------------------------------===//

static const uint16_t ArgRegs[] = {
  T64::R2, T64::R3, T64::R4,
  T64::R5, T64::R6, T64::R7, T64::R8, T64::R9, T64::R10
};

static const unsigned NumArgRegs = array_lengthof(ArgRegs);

static bool CC_rvex_Assign(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
...
//===----------------------------------------------------------------------===//
// Helper functions implementing features of rvexTargetLowering
//===----------------------------------------------------------------------===//
...
SDValue rvexTargetLowering::LowerConstantPool(SDValue Op,
                                                SelectionDAG &DAG) const {
...
SDValue rvexTargetLowering::LowerVAARG(SDValue Op, SelectionDAG &DAG) const {
...
SDValue rvexTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                      SelectionDAG &DAG) const {
...
/*
SDValue rvexTargetLowering::LowerFRAMEADDR(SDValue Op, SelectionDAG &DAG,
...
SDValue rvexTargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
  MachineFrameInfo *MFI = DAG.getMachineFunction().getFrameInfo();
...
*/
MachineBasicBlock *rvexTargetLowering::
...
EmitSTORE_FPWithCustomInserter(MachineInstr *MI, MachineBasicBlock *MBB) const {
  ...
  if(MFI->hasCalls()) {
    ...
    BuildMI(*MBB, MBBI, DL, TII->get(T64::ADDI), Reg).addReg(T64::StackPointer)
                                                     .addImm(4);
    //BuildMI(*MBB, MBBI, DL, TII->get(T64::SW)).addReg(Reg).addReg(T64::FramePointer);
  }
...
rvexTargetLowering::rvexTargetLowering(TargetMachine &TM)
  ...
  // rvex doesn't have sext_inreg, replace them with shl/sra
  ...
  // rvex has no DIV or REM or DIVREM operations.
  ...
  // rvex has no select or select_cc: expand to set_cc.
...
LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  ...
  case ISD::GlobalTLSAddress:
    llvm_unreachable("TLS not implemented for rvex.");
...
/*
  ...
  const unsigned StackOffset = DAG.getMachineFunction().
                               getInfo<rvexMachineFunctionInfo>()->
                               getArgAreaOffset();
  ...
  if (MF.getFunction()->hasStructRetAttr()) {
    ...
    rvexMachineFunctionInfo *FI = MF.getInfo<rvexMachineFunctionInfo>();
...
SDValue rvexTargetLowering::LowerCall(CallLoweringInfo &CLI,
                                        SmallVectorImpl<SDValue> &InVals) const{
  //FIXME: rvex target does not yet support tail call optimization.
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_rvex);
...
bool rvexTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
